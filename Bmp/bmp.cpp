...
void CreateBmp24 (char *fname, RGBTRIPLE* color, int Width, int Height);
                     ...
                     int       nCmdShow)
{
	...
	int Width = 100;
	int Height = 100;
	RGBTRIPLE* color24 = new RGBTRIPLE[Width*Height];

	for(int j=0; j<Height-20; j++)
	{
		for(int i=0; i<Width-20; i++)
		{
			color24[i+Width*j].rgbtRed = (111-3*j)%256;
			color24[i+Width*j].rgbtGreen = (20+2*j)%256;
			color24[i+Width*j].rgbtBlue = (160+5*j)%256;
		}
		for(int i=Width-20; i<Width; i++)
		{
			color24[i+Width*j].rgbtRed = 0;
			color24[i+Width*j].rgbtGreen = 0;
			color24[i+Width*j].rgbtBlue = 0;
		}
	}
	...
	CreateBmp24 ("test2.bmp", color24, Width, Height);
...
void CreateBmp555 (char *fname, WORD color)
{
	...
	hFile = CreateFileA (fname, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, 0, NULL);
...
void setRGBColors(RGBTRIPLE* color, int* Points_input, int Width, int Height)
{
...
void CreateBmp24 (char *fname, RGBTRIPLE* color, int Width, int Height)
{
	...
	for (i = 0; i < Height; i++)
	{
		for (j = 0; j < Width; j++)
		{
			WriteFile (hFile, &color[j + Width*i ], sizeof(RGBTRIPLE), &RW, NULL);
		}
		// Выровняем по границе
