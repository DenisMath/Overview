...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	KWnd mainWnd("ModelessDlg", hInstance, nCmdShow, WndProc,
		NULL, 100, 100, 500, 500);	
for(int i = 1; i<5; i++)
{
ShowWindow(CreateWindow("KWndClass", strcat("ModelessDlg","temp"), WS_OVERLAPPEDWINDOW, 300+10*i, 300+10*i, 500, 500, NULL,
   NULL,
   hInstance,
   NULL), nCmdShow); 
}
	while (GetMessage(&msg, NULL, 0, 0))  {
		if (!(IsDialogMessage(hModelessDlg, &msg)||IsDialogMessage(hModelessDlg1, &msg))) {
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
        ...
        hModelessDlg1 = CreateDialog(GetModuleHandle(NULL),
			MAKEINTRESOURCE(IDD_MODELESS), hWnd, ModelessDlgProc);
		AdjustDlgPlace(hWnd, hModelessDlg);
        //AdjustDlgPlace(hWnd1, hModelessDlg);
		break;
