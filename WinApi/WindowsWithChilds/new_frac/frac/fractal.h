...
void convertFractalToPairVector(const Fractal &f, std::vector<std::pair<float,float>> &VectorPair )
{
	VectorPair.clear();
	int temp1 = f.fracset.size();
	VectorPair.reserve(temp1);
	std::pair<float,float> temp;
	FractalType::const_iterator at = f.fracset.begin();
...
void wConvertFractalToPairVector(Fractal &f, std::vector<std::pair<float,float>> &VectorPair, std::vector<float> &Tension )
{
	VectorPair.clear();
	Tension.clear();
	int temp1 = f.fracsetW.size();
	VectorPair.reserve(temp1);
	Tension.reserve(temp1);
	std::pair<float,float> temp;
